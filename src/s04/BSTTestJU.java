...
public class BSTTestJU {
  ...
  public void testConstructor() {
    ...
    for (int n=0; n<maxSize; n++) {
      ...
      for(int i=0; i<n; i++) tab[i]=new Integer(i-offset);
      t = new BST<Integer>(tab);
  ...
  // ------------------------------------------------------------
  // ------------------------------------------------------------
  static private boolean shouldLog = false;
  // ------------------------------------------------------------
  static private void rndAddRm(Random r, BST<Integer> s, BitSet bs, int i) {
    String log="";
    if (r.nextBoolean()) { 
      s.add   (new Integer(i)); bs.set  (i);
      log += "\n--- add "+i+"\n" + s.tree.toReadableString();
    } else {
      s.remove(new Integer(i)); bs.clear(i);
      log += "\n--- remove "+i+"\n" + s.tree.toReadableString();
    }
