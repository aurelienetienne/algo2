...
public class GeneralSearchTreeMap<K extends Comparable<K>, V> {
	...
	public V get(K key) {
		...
		GSTNode node = root;
		while (!node.isLeaf()) {
			int nbkey = node.nbOfKeys();
			
			for (int i = 0; i < nbkey; i++) {
				K crtKey = node.getKey(i);

				if (key.compareTo(crtKey) < 0) {
					node = node.getSon(i);
					break;
				}
				if (i == nbkey - 1) {
					node = node.getSon(nbkey);
					break;
				}
			}
		}
	...
	private int nbOfKeysInRange(GSTNode n, K min, K max) {
		...
		if (!n.isLeaf()) {
			for (int i = 0; i < nbKey; i++) {
				...
				if ((min.compareTo(crtKey) <= 0)) {
					count += nbOfKeysInRange(n.getSon(i), min, max);

					if (max.compareTo(crtKey) < 0) {
						break;
					}
				}

				if (i == nbKey - 1 && max.compareTo(crtKey) > 0 )
					count += nbOfKeysInRange(n.getSon(nbKey), min, max);
			}
		} else {
			for (int i = 0; i < nbKey; i++) {
				K crtKey = n.getKey(i);
				if ((min.compareTo(crtKey) <= 0) && (max.compareTo(crtKey) > 0)) {
					count++;
				}
			}
		}
	...
	class GSTNode {
		...
		public GSTNode(GSTNode n, int from, int to) {
			this(); isLeaf_DONT_USE=n.isLeaf_DONT_USE;
			if (isLeaf_DONT_USE) {
				for(int i=from; i<to; i++) {
					keys_DONT_USE.add(n.getKey(i)); vals_DONT_USE.add(n.getVal(i));
				}
			} else {
		...
		public GSTNode(K key, V val) {
			this();
			isLeaf_DONT_USE=true;
			keys_DONT_USE.add(key); vals_DONT_USE.add(val);
		}
		// ------------------------------------------------------------
		public boolean isLeaf()        { return isLeaf_DONT_USE;}
		public V       getVal(int i)   { return vals_DONT_USE.get(i);}
		public GSTNode getSon(int i)   { return sons_DONT_USE.get(i);}
		public int     nbOfSons()      { return isLeaf_DONT_USE ? 0:nbOfKeys()+1;}
		// ------------------------------------------------------------
		public int nbOfKeys() {
		...
		private void putInLeaf_DONT_USE(K key, V val) {
			...
			for (i=0; i<keys_DONT_USE.size(); i++) {
				...
				if (cmp==0) {vals_DONT_USE.set(i, val); return; }
				if (cmp>0) break;
			}
			keys_DONT_USE.add(i,key); vals_DONT_USE.add(i,val);
		}
		// ------------------------------------------------------------  
		...
		private void borrowFromLeft_DONT_USE(GSTNode crt, GSTNode left) {
			if (crt.isLeaf()) {
				...
				crt.keys_DONT_USE.add(0,k); crt.vals_DONT_USE.add(0,v);
			} else {
		...
		private void borrowFromRight_DONT_USE(GSTNode crt, GSTNode right) {
			if (crt.isLeaf()) {
				...
				crt.keys_DONT_USE.add(k); crt.vals_DONT_USE.add(v);
			} else {
		...
		private void removeFromLeaf_DONT_USE(K key) {
			...
			if (i==-1) return;
			keys_DONT_USE.remove(i); vals_DONT_USE.remove(i);
		}
		// ------------------------------------------------------------  
		...
		private void remove_DONT_USE(K key) {
			if (isEmpty()) return;
			if (isLeaf()) { removeFromLeaf_DONT_USE(key); return; }
		...
		private void put_DONT_USE(K key, V val) {
			if (isLeaf_DONT_USE) { putInLeaf_DONT_USE(key, val); return;}
		...
		private void split_DONT_USE() {
			...
			keys_DONT_USE.clear(); vals_DONT_USE.clear();
			isLeaf_DONT_USE=false;
			sons_DONT_USE.clear();
			sons_DONT_USE.add(a); sons_DONT_USE.add(b);
		}
		// ------------------------------------------------------------ 
		public String toString() { return toString_DONT_USE("");}
